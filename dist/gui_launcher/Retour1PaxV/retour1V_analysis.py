...
df['Price 1PAX+1VEH'] = pd.to_numeric(df['Price 1PAX+1VEH'].str.replace('.', '').str.replace(',', '.'), errors='coerce')

# Group by 'Trajet', 'Company', and 'Date' and calculate summary statistics
summary = df.groupby(['Trajet', 'Company', 'Date'])['Price 1PAX+1VEH'].agg(['mean', 'min', 'max'])

# Create a new Excel file and add summary statistics
output_file = 'retour1V_analysis.xlsx'
workbook = Workbook()
...
for trajet in df['Trajet'].unique():
    ...
    for company in trajet_df['Company'].unique():
        ...
        line_chart.y_axis.title = 'Price 1PAX+1VEH'
